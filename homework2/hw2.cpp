...
void get_v0(ifstream &file, double &vx, double &vy) {
    string v1, v2;
    file >> v1 >> v2;
    vx = stod(v1); // ??? то есть я внутри void могу изменить значение чего-либо с помощью указателя?
    vy = stod(v2);
}
...
void funct(double x0, double h0, double vx, double vy, vector<double> &X, vector<double> &H, int &res, int delta) {
    double t, y;
    for (int i = res; (i > -1 && i < X.size()); i = i + delta) // ??? несколько условий, и можно будет внутри цикла остановить цикл с помощью к примеру -1?
    {
        t = (X[i + delta] - x0) / (delta * vx);
        y = H_t(h0, vy, t);
        if (H[i + delta] < y) { // если высота столба меньше чем наш y в данный момент
            res += delta;
        }
        else if ((y < 0) || (res == 0)) { // если точка столкнулась со столбом или точка упала
            return; // ??????
        }
        else
        {
